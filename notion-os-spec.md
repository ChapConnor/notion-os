...
**Status: all four phases complete. Build plan in `plan.md`.**
...
- Phase 2 — Sync architecture: complete (§2), confirmed 2026-08-24 — resolutions in §2.8
- Phase 3 — Build plan: **complete — see `plan.md`**
...
| Category | select | Flat select seeded from `categories.yml` (seeded in turn from personal_finance_v2.xlsx's Keyword Rules sheet). **Naming convention is load-bearing for Kind**: plain names for spend (Groceries, Dining, Transport, Housing, Phone *(added §2.8)*, Subscriptions, Health, Golf, Gifts, Travel, Shopping, Fees, Cash, Misc, Uncategorized); `Income — Salary`, `Income — Other`; `Contribution — TFSA/RRSP/FHSA/Taxable`; `Transfer — CC Payment`, `Transfer — Internal`. The one property humans edit. |
...
### 2.8 Phase 2 checkpoint — resolved 2026-08-24

1. **Notification grace kept** (2 quiet, notify on the 3rd; auth always immediate). Resource cost is nil — it's a counter in local state; the choice is purely about noise, and the grace period was designed against the documented nag-fatigue trigger.
2. **Scotia Visa exports CSV directly** — no PDF conversion step in the monthly runbook; `inspect` still arbitrates between the two candidate CSV shapes on first contact.
3. **Bank export reach is 1 year** (both banks). Consequence: `deadman_window_months` widens **6 → 12** (still one 1–2-request query; an ignored month can now never age out of the nag while it's still recoverable from the bank) and `export_horizon_warn_d` recalibrates **75 → 300**.
4. **Categories**: Cineplex/Ticketmaster → Misc; SHAW/TELUS/ROGERS → new plain category **Phone** (added to §1.1.4's select and the taxonomy; Kind = Expense by the plain-name convention); brokerage is **RBC** — contribution rules watch RBC Direct Investing / InvestEase transfer descriptors, and all five balance columns read from RBC statements (the sitting touches exactly two portals: RBC + Scotia). Shoppers→Health went unanswered — seed default (Health) stands, flagged for the build-weekend categories.yml review.
5. **Mac is usually asleep, not shut down** — the run-on-wake model is the right fit as designed; schedules confirmed (daily 07:15 / Mon 07:40 / Fri 09:00).

**Phase 2 checkpoint resolved — architecture confirmed. Phase 3 (build plan) follows in `plan.md`.**

---

## Phase 3 — Build plan

The build plan lives in **`plan.md`** (per §7 output conventions): nine sessions with checkboxes, ordered so the social CRM works end-to-end after the first evening — before any code exists; every step labelled `[hand · Notion UI]`, `[hand · terminal]`, `[hand · bank/phone]`, or `[build]`; per-session acceptance checks; a "when the scale arrives" checklist; and the deferred v2 ledger carried over from §1.9. The plan introduces no new design — every line traces to §1 or §2.
